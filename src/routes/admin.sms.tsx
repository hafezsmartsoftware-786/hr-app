...
import { Search, Send, CheckCircle2, XCircle, Clock, AlertCircle, RefreshCw, Phone, Mail, ShieldAlert, Key } from "lucide-react";
...
function SmsPage() {
  ...
  const sendMutation = useMutation({
    mutationFn: async (args: { mobile: string; email: string; password?: string }) => {
      return sendFn({
        data: {
          mobile: args.mobile,
          email: args.email,
    ...
    onError: (err: any) => {
      toast.error(err?.message || "Send SMS error");
    },
  });
