...
import {
  ...
  BadgeCheck, Clock, TrendingUp, Users, BarChart3, Star, StickyNote,
} from "lucide-react";
...
export const Route = createFileRoute("/finance/profile")({
  component: FinanceProfilePage,
});

// ─── Tab Types ────────────────────────────────────────────────────────────────
type Tab = "overview" | "settings" | "devices" | "access";

function FinanceProfilePage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="mb-1 min-w-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  {realProfile?.emp_code && (
                    <span className="rounded-lg bg-muted px-2.5 py-0.5 font-mono text-xs font-semibold text-foreground border border-border/60">
                      #{realProfile.emp_code}
                    </span>
                  )}
                  <span className="inline-flex items-center gap-1 rounded-full bg-brand/10 px-2.5 py-0.5 text-xs font-bold text-brand border border-brand/20">
                    <Shield className="h-3 w-3" /> Finance
                  </span>
            ...
            <div className="flex flex-wrap gap-2 sm:flex-nowrap">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl bg-muted/40 px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted transition-colors"
              >
                <Lock className="h-3.5 w-3.5" /> Change Password
              </button>
              <button
                onClick={async () => { await signOut(); window.location.href = "/auth"; }}
                className="inline-flex items-center gap-1.5 rounded-xl bg-destructive/5 px-3 py-2 text-xs font-semibold text-destructive hover:bg-destructive/10 transition-colors"
              >
      ...
      {/* ── Tab Bar ────────────────────────────────────────────────────────── */}
      <div className="flex overflow-x-auto rounded-2xl bg-card p-1 gap-1 shadow-md">
        {tabs.map((tab) => (
          <button
            key={tab.id}
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          {/* Quick Links */}
          <div className="rounded-2xl bg-card overflow-hidden shadow-md lg:col-span-2">
            <div className="px-5 py-4 border-b border-border/40">
              <h2 className="flex items-center gap-2 text-sm font-bold text-foreground">
            ...
            <div className="grid grid-cols-2 divide-x divide-border/40 sm:grid-cols-4">
              {[
                { to: "/finance/payroll", icon: Wallet, label: "Payroll", color: "text-indigo-500 bg-indigo-500/10" },
                { to: "/finance/payroll-settings", icon: Settings, label: "Payroll Settings", color: "text-emerald-500 bg-emerald-500/10" },
                { to: "/finance/advances", icon: Banknote, label: "Advances", color: "text-amber-500 bg-amber-500/10" },
                { to: "/finance/sticky-notes", icon: StickyNote, label: "Sticky Notes", color: "text-purple-500 bg-purple-500/10" },
              ].map((link) => (
      ...
      {activeTab === "settings" && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-2xl bg-card px-5 py-4 flex items-center justify-between shadow-md">
              <div className="flex items-center gap-3">
            ...
            <button
              onClick={async () => { await signOut(); window.location.href = "/auth"; }}
              className="w-full flex items-center justify-center gap-2 rounded-2xl bg-destructive/5 px-4 py-4 text-sm font-bold text-destructive hover:bg-destructive/10 transition-colors shadow-md"
            >
          ...
          <div className="p-5 space-y-4">
            ...
            <div className={`rounded-2xl border-2 p-4 ${
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-background text-muted-foreground border border-border">
              ...
              <div className="mt-4">
                {deviceStatus === "approved" ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <button onClick={() => handleRemove(deviceId)} className="rounded-xl bg-background px-4 py-2 text-xs font-semibold hover:bg-muted transition-colors shadow-sm">
                      Remove & Re-register
                    </button>
                  </div>
                ) : (
                  ...
                  {myDevices.filter((d: any) => d.id !== deviceId).map((d: any) => (
                    <div key={d.id} className="flex items-center justify-between gap-3 rounded-xl bg-muted/30 p-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-background text-muted-foreground border border-border">
                          <Smartphone className="h-4 w-4" />
                        </span>
                        <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button onClick={() => handleRemove(d.id)} className="rounded-lg bg-background px-2.5 py-1 text-xs font-semibold hover:bg-muted transition-colors shadow-sm">
                          Remove
                        </button>
                      </div>
                    </div>
          ...
          <div className="rounded-2xl bg-card overflow-hidden shadow-md">
            ...
            <div className="p-4">
              {myLocations.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border/40 bg-muted/30 p-6 text-center">
                ...
                <div className="space-y-2">
                  {myLocations.map((l) => (
                    <div key={l.id} className="flex items-center justify-between rounded-xl bg-muted/40 px-4 py-3">
                      <div className="flex items-center gap-3">
                        <span className="grid h-8 w-8 place-items-center rounded-lg bg-background text-brand shadow-sm">
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex gap-2 pt-2">
          <button type="button" onClick={onClose} className="flex-1 rounded-xl bg-card py-2.5 text-sm font-semibold hover:bg-muted transition-colors shadow-sm">
            Cancel
          </button>
    ...
    <label className="block space-y-1.5">
      ...
      <div className="relative">
        <input
          ...
          className="w-full rounded-xl bg-background px-3 py-2.5 pr-10 text-sm outline-none focus:ring-2 focus:ring-brand/20 transition-all shadow-sm"
          autoComplete="off"
        />
        <button
          type="button"
